...
from slp.load_lexicons.get_all_lexicons import LexiconFeatures
...
class WordAttNet(nn.Module):
    def __init__(self, dict_size, diction, hidden_size=300):
        ...
        self.gru = nn.GRU(380, 380, bidirectional = True, batch_first=True) #changed hidden & input size.
    ...
    def forward(self, inputs, lengths, hidden_state, idx2word):
        output = self.lookup(inputs)

        output_lex = self.lexicons(inputs, idx2word, padding_len=output.shape[1]).float()

        output = torch.cat((output, output_lex), axis=2) #to concatenation tis eisodou me ta lexica
...
class SentAttNet(nn.Module):
    def __init__(self,hidden_size=300, num_classes=0):
        ...
        self.gru = nn.GRU(2 * hidden_size, 380, bidirectional=True, batch_first=True) #changed hidden size
    ...
    def forward(self, inputs, lengths, titles, hidden_state):
...
class HierAttNet(nn.Module):
    def __init__(self, hidden_size, batch_size, num_classes, max_sent_len, dict_size, diction, idx2word):
    ...
    def forward(self, inputs, lengths, titles, title_lengths):
        ...
        for i in text:
            ...
            output_text, self.word_hidden_state = self.word_att_net_text(i, word_lengths, self.word_hidden_state, self.idx2word) #[8,600]
            output_list_text.append(output_text)
            self.word_hidden_state = repackage_hidden(self.word_hidden_state)

        output_title, self.word_hidden_state = self.word_att_net_text(titles, title_lengths, self.word_hidden_state, self.idx2word) #[8,600]
        self.word_hidden_state = repackage_hidden(self.word_hidden_state)
...
#        import pdb; pdb.set_trace()
        ...
        output, self.sent_hidden_state = self.sent_att_net(output_list_text, lengths, output_title, self.sent_hidden_state)
