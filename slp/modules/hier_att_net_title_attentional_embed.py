    ...
    def forward(self, inputs, lengths, hidden_state, idx2word, lex_size, is_title=False):
        ...
        try:
            ...
            for i in output[0]:
                if i.shape[0]!=300:
                    import pdb; pdb.set_trace()
        f_output = self.unpack(f_output, lengths, max_length = 150)

#        f_output = f_output.view(batch_size, num_sentences, -1)
...
class HierAttNet(nn.Module):
    ...
    def forward(self, inputs, number_of_sentences, length_of_sentences, titles, title_lengths):
        # inputs = (B, S, W)
        import pdb; pdb.set_trace()
        ...
        length_of_sentences = length_of_sentences.view(-1)

        temp = inputs[:100]
        temp_lengths = length_of_sentences[:100]

        output_text, self.word_hidden_state = self.word_att_net_text(temp, temp_lengths, self.word_hidden_state, self.idx2word, self.lex_size, is_title=False)


# -----
#        for i in text:

#            word_lengths = i.size(1) - (i==0).sum(dim=1) # to mikos tis kathe protasis apo tis 8 tou batch kathe fora
...
#        import pdb; pdb.set_trace()
        ...
        output, self.sent_hidden_state = self.sent_att_net(output_list_text, number_of_sentences, self.sent_hidden_state)
