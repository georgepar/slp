...
def augment_parser(parser):
    ...
    parser.add_argument(
        '--logging-dir', dest='logging_dir',
        type=str, default=None,
        help='Path to experiment logging directory')

    parser.add_argument(
        '-lr', '--learning-rate', dest='optimizer.learning_rate',
        type=float, default=None,
        help='Set the learning rate')

    parser.add_argument(
        '-emb', '--embeddings-path', dest='embeddings.path',
        type=str, default=None,
        help='Path to word embeddings')

    parser.add_argument(
        '--embeddings-dim', dest='embeddings.dim',
        type=int, default=None,
        help='Dimensionality of word embeddings')

    parser.add_argument(
        '--embeddings-dropout', dest='embeddings.dropout',
        type=float, default=None,
        help='Embeddings Dropout')

    parser.add_argument(
        '--embeddings-finetune', dest='embeddings.finetune',
        action="store_true",
        help='Finetune embeddings')

    parser.add_argument(
        '-b', '--batch-size', dest='dataloaders.batch_size',
        type=int, default=None,
        help='Set the batch size')

    parser.add_argument(
        '-j', '--num-workers', dest='dataloaders.num_workers',
        type=int, default=None,
        help='Number of workers for data loading')

    parser.add_argument(
        '--pin-memory', dest='dataloaders.pin_memory',
        default=None, action='store_true',
        help='Pin CUDA memory for data loading')

    parser.add_argument(
        '--accumulation-steps', dest='trainer.accumulation_steps',
        type=int, default=None,
        help='Gradient accumulation steps to simulate large batch size')

    parser.add_argument(
        '--clip-grad-norm', dest='trainer.clip_grad_norm',
        type=float, default=None,
        help='Gradient accumulation steps to simulate large batch size')

    parser.add_argument(
        '--patience', dest='trainer.patience',
        type=int, default=None,
    ...
    parser.add_argument(
        '-e', '--max-epochs', dest='trainer.max_epochs',
        type=int, default=None,
        help='Maximum number of epochs to run')

    parser.add_argument(
        '-v', '--validate-every', dest='trainer.validate_every',
        type=int, default=None,
        help='Validate every N epochs')

    parser.add_argument(
        '--parallel', dest='trainer.parallel',
        default=None, action='store_true',
        help='Run on all available GPUs')

    parser.add_argument(
        '--non-blocking', dest='trainer.non_blocking',
        default=None, action='store_true',
        help='Load data into GPU without blocking')

    parser.add_argument(
        '--retain-graph', dest='trainer.retain_graph',
        default=None, action='store_true',
        help='Retain computational graph. Might want to set for RNNs')

    parser.add_argument(
        '-exp', '--experiment', dest='trainer.experiment_name',
        type=str, default=None,
        help='Experiment name')

    parser.add_argument(
        '--checkpoint-dir', dest='trainer.checkpoint_dir',
        type=str, default=None,
        help='Path to checkpoint directory')

    parser.add_argument(
        '--load-model', dest='trainer.model_checkpoint',
        type=str, default=None,
        help='Resume from model checkpoint / Transfer learning')

    parser.add_argument(
        '--load-optimizer', dest='trainer.optimizer_checkpoint',
        type=str, default=None,
