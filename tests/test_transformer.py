...
def test_inner_layers_output_size():
    ...
    e = model.transformer_block.encoder(
        x, attention_mask=mask1)
    assert e.size() == (1, len(sentence) - 1, hidden_size)

    el = model.transformer_block.encoder.encoder[0](
        x, attention_mask=mask1)
    assert el.size() == (1, len(sentence) - 1, hidden_size)

    s1 = model.transformer_block.encoder.encoder[0].l1(
        x, attention_mask=mask1)
    ...
    d = model.transformer_block.decoder(
    ...
    s3 = model.transformer_block.decoder.decoder[0].in_layer(
        y, attention_mask=mask2)
