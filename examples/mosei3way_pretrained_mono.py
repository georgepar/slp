...
from slp.modules.multimodal import AudioVisualTextClassifier, AudioClassifier, GloveClassifier, VisualClassifier
...
if __name__ == "__main__":
    ...
    #     num_classes=1,
    # )
    # a_model.load_state_dict(torch.load(C["audio_dir"])["model"])
    ...
    #     num_classes=1,
    # )
    # t_model.load_state_dict(torch.load(C["text_dir"])["model"])
    if C["from_pretrained"]:
        model.from_pretrained(
            audio_path=C["audio_dir"],
            text_path=C["text_dir"],
            visual_path=C["visual_dir"],
        )
    print(model)
